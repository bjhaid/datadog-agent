...
func (p *ProcessCheck) runRealtime(cfg *config.AgentConfig, groupID int32) (*RunResult, error) {
	...
	containers, lastContainerRates, pidToCid, err = p.containerProvider.GetContainers(cacheValidityRT, p.lastContainerRatesRT)
	if err == nil {
		p.lastContainerRatesRT = lastContainerRates
	} else {
